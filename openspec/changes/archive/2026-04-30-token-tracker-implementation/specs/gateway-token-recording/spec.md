...
The gateway-service SHALL call billing-service `RecordUsage` RPC after completing a non-streaming LLM request, using token counts from the provider response.

#### Scenario: Non-streaming request completes
- **WHEN** a non-streaming request to a provider completes
- **THEN** the gateway extracts prompt_tokens and completion_tokens from `ForwardRequestResponse.TokenCounts`
- **AND** calls `billingClient.RecordUsage()` with:
  - user_id: from JWT context
  - group_id: from ValidateAPIKey response (first group)
  - provider_id: from router selection
  - model: from the request
  - prompt_tokens, completion_tokens: from provider response

#### Scenario: Multiple providers called
...
The gateway-service SHALL accumulate token counts across all SSE chunks and call `RecordUsage` after the stream completes.
...
The gateway-service SHALL extract token counts from provider responses for both non-streaming and streaming flows.
