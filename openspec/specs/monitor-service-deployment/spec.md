...
Deployment specifications for the monitor-service microservice with gRPC interface and Docker Compose orchestration.
## Requirements
### Requirement: Docker multi-stage build
The monitor-service SHALL be built using a multi-stage Dockerfile that produces a minimal production image based on alpine.
...
The monitor-service container SHALL run as a non-root user for security.
...
The monitor-service SHALL load configuration from a YAML file with environment variable resolution.
...
The monitor-service SHALL implement gRPC health checking protocol.
...
The monitor-service SHALL be deployable via Docker Compose with health check configuration.

#### Scenario: Service starts in Docker Compose
- **WHEN** `docker-compose up` is run
- **THEN** the monitor-service SHALL start on gRPC port 50055
