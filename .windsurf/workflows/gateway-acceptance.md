...
Integration tests must be run against live services in Docker Compose environment:
1. Rebuild environment: `make down && make clean-images && make up`
2. Wait for all services healthy (check `docker compose ps`)
3. Run integration tests: `make test-integration` or service-specific tests
4. Verify cross-service gRPC calls work correctly
...
**Environment Rebuild for Verification:**
To ensure clean state for acceptance verification:
```bash
make down              # Stop all services
make clean-images      # Remove old Docker images
...
**Rebuild environment for clean verification:**
```bash
make down
make clean-images
make up
...
# Or test specific service interactions
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer $API_KEY" \
  -d '{"model":"ollama:llama2","messages":[{"role":"user","content":"Hello"}]}'
```

Verify all services respond correctly and cross-service gRPC calls work.
